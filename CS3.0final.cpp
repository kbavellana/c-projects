	...
	//search for the NEXT! job to be put in the process
		...
		else if(j!=0)
		{
			for(x=0;x<j;x++)
				...
				for(i=0;i<jobb;i++)
				{
					if((job[i].Burst==storage[0]) && (job[i].Arrival<=timeline) && (job[i].skip==false))
					{
						p=i;
						Time_start  = Time_finish;
						timeline   += job[p].Burst;
	...
	while(timeline!=Total_burst)
	{
		...
		else if(j!=0)
		{
			for(x=0;x<j;x++)
				...
				for(i=0;i<jobb;i++)
				{
					if((job[i].priority==storage[0]) && (job[i].Arrival<=timeline) && (job[i].skip==false))
					{
						p=i;
						Time_start  = Time_finish;
						timeline   += job[p].Burst;
	...
	while(timeline!=Total_burst)
	{
		...
		else if(j!=0)
		{
			...
			//end of sorting		
				...
				for(i=0;i<jobb;i++)
				{
					if((job[i].priority==storage[0]) && (job[i].skip==false) && (job[i].Arrival<=timeline))
					{	p=i;
						//process start
...
printf("\ntimeline:%d",timeline); getch();
		...
		if(j!=0)
		{
			...
			//end of sorting		
				for(i=0;i<jobb;i++)
				{
					if((job[i].temp_burst==storage[0]) && (job[i].Arrival<=timeline) && (job[i].skip==false))
					{	p=i; printf("job#%d ang nasa process",p+1);
						if((p != prev_j) && (ctr==1) && (job[prev_j].skip==false))
						{
									job[prev_j].deduction+=(Time_finish-Time_start);
									job[prev_j].swits=true;
...
{int jobb=0,Total_burst=0,ctr=0,arr[30],t_idle[30];
	...
	while(timeline!=Total_burst)
	{
		...
		//end of sorting		
				...
				for(i=0;i<jobb;i++)
				{
					if((job[i].time_out==storage[0]) && (job[i].Arrival<=timeline) && (job[i].skip==false))
					{	p=i;
						if((p != prev_j) && (ctr==1) && job[prev_j].skip==false)
						{
									job[prev_j].deduction+=(Time_finish-Time_start);
